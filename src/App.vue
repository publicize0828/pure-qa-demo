...
const llmModel = ref(LLM_MODEL.value)
...
function showSdkError(data: any) {
  ...
  if (msg) { errorToast.value?.show(msg) }
}
...
<template>
  <div class="app-root">
    ...
    <div v-if="asrText" class="asr-text-overlay">{{ asrText }}</div>
    <!-- 全屏画布 -->
    <div id="avatar-canvas" class="canvas-fullscreen"></div>

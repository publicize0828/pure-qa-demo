...
export function useASR() {
  ...
  async function connectWS(config: any): Promise<WebSocket> {
    ...
    return new Promise<WebSocket>((resolve, reject) => {
      ...
      socket.onopen = () => { console.log('[ASR] WebSocket 已连接'); reconnectCount = 0; resolve(socket) }
      socket.onerror = (e) => { console.error('[ASR] WebSocket 错误', e); reject(new Error('ASR WebSocket 连接失败')) }
      socket.onclose = (e) => {
        console.log('[ASR] WebSocket 关闭', e.code, e.reason)
        ...
        if (ws && isListening.value && reconnectCount < MAX_RECONNECT) {
          ...
          reconnectCount++
          console.log('[ASR] %ds 后第 %d 次重连…', delay / 1000, reconnectCount)

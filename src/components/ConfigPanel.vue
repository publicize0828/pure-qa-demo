...
function copyInvite() {
  navigator.clipboard.writeText('JU6AD24X9V')
  copied.value = true
  ...
  <div class="config-float">
    ...
    <div v-show="configOpen" class="config-body">
      ...
      <template v-if="!isModelScope">
        <div class="cfg-section">
          ...
          <details class="cfg-help">
            ...
            <p>邀请码：<b class="invite-code" @click="copyInvite">JU6AD24X9V</b><span v-if="copied" class="copied-hint"> 已复制</span>（注册时填入可获 1000 积分）</p>
      ...
      <template v-else>
        <div class="cfg-section">
          <label>数字人驱动</label>
          <button class="btn-fill" @click="openCredModal">
            ✨ 连接数字人
          </button>
        </div>
      </template>
